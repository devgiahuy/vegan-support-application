...
> {
  toModel(dto: HealthProfileDto | null | undefined): HealthProfile {
    ...
    const sex = safeEnum(pickField(dto, ['sex'], BiologicalSex.MALE), BiologicalSex, BiologicalSex.MALE);
    const activityLevel = safeEnum(
      pickField(dto, ['activityLevel', 'activity_level'], ActivityLevel.SEDENTARY),
      ActivityLevel,
      ActivityLevel.SEDENTARY
    );
    const bmi = safeNumber(pickField(dto, ['bmi'], 0));
    const category = heightCm > 0 && weightKg > 0 ? calcBmi(weightKg, heightCm).category : 'Chưa có dữ liệu';
    ...
    return {
      ...
      dataSource: safeEnum(
        pickField(dto, ['dataSource', 'data_source'], HealthDataSource.MANUAL),
        HealthDataSource,
